///////////////////////////////////////////////////////////////////////////////
// 说明：全局脚本主题来源于Z-Blog（朱煊）
// 版权所有:    RainbowSoft Studio
// 技术支持:    rainbowsoft@163.com
// 修改补充： 天缘（Lovoner@gmail.com 博客：http://www.metsky.com）
///////////////////////////////////////////////////////////////////////////////
...
{		
	...
	{   
		...
		{   
			alert("Refused by your browsers!\n Type 'about:config' in browser's URL address and Press Enter\n Set 'signed.applets.codebase_principal_support'='true'");   
		}   
		...
		alert("Copy Success, Press CTRL+V to paste.");
	}   	
}
...
//插入引用
function InsertQuote(intCmtId) {
	getem("inpQid").value=intCmtId;
	getem("canelquote").style.display='';
}
...
function CanelQuote() {
	getem("inpQid").value='';
	getem("canelquote").style.display='none';
}


//插入回复
function InsertReply(strAuthorId,strLinkId) 
{	
	if(!objActive){objActive="txaArticle"};
	var strAuthor = getem(strAuthorId).innerHTML;//read author

	InsertText(objActive,'<a href="#'+strLinkId+'">@'+strAuthor+':</a>',false);
	
	if(getem("inpQid").value!='')
		CanelQuote();
}
...
function VerifyMessage() {
	...
	var strName=getem("inpName").value;
	var strEmail=getem("inpEmail").value;
	var strHomePage=getem("inpHomePage").value;
	var strArticle;
	
	if(getem("txaArticle").value){
		strArticle=getem("txaArticle").value;
	}
	else{
		strArticle=getem("txaArticle").innerText;
	}
	...
	if(strHomePage=="http://")
		strHomePage="";
	if(strHomePage!=""){
		re = new RegExp("((?:http|https|ftp|mms|rtsp)://(&(?=amp;)|[A-Za-z0-9\./=\?%_&#\+\-])+$)","ig");
		if (!re.test(strHomePage)){
			alert(str04);
			return false;
		}
	}
	...
	getem("inpArticle").value=strArticle;
	getem("inpLocation").value=parent.window.location.href;
	if(getem("frmSumbit").action.search("act=cmt")>0){
		strFormAction=getem("frmSumbit").action;
	}

	var bolRemember=getem("chkRemember").checked;
	...
	//ajax comment begin
	$.post(getem("frmSumbit").action,
		{
		"inpAjax":true,
		"inpID":$("#inpId").val(),
		"inpQid":$("#inpQid").val(),
		"inpVerify":(getem("inpVerify")?$("#inpVerify").val():""),
		"inpEmail":strEmail,
		"inpName":strName,
		...
		function(data){
			...
			if((s.search("faultCode")>0)&&(s.search("faultString")>0))
			{
				...
				if(strFormAction){
					getem("frmSumbit").action=strFormAction;
				}
				$("#txaArticle").val("");
				
				CanelQuote();//成功处理才取消引用状态
			}
			if(getem("inpVerify")){
				$("#inpVerify").val("");
				var objImageValid=$("img[@src^='"+strHost+"/include/validcode.php?act=cmtVerify']");
				objImageValid.attr("src",strHost+"/include/validcode.php?act=cmtVerify"+"&random="+Math.random());
			}
...
function loadpage(url) {
	//ajax comment begin
	$.get(strHost+decodeURIComponent(url),function(data){
			var s =data;
			if((s.search("faultCode")>0)&&(s.search("faultString")>0)) {
				alert(s.match("<string>.+?</string>")[0].replace("<string>","").replace("</string>",""))
			}else{
				var b=getem("divLoadmore");
				$(data).insertBefore(b);
				var p=getem("loadmoreREF");
...
function LoadRememberInfo() {
	...
	if(bolRemember=="true"){
		...
		if(strName){getem("inpName").value=strName;};
		if(strEmail){getem("inpEmail").value=strEmail;};
		if(strHomePage){getem("inpHomePage").value=strHomePage;};
		if(bolRemember){getem("chkRemember").checked=bolRemember;};

	}

	if(GetCookie("MemOL")){
		var arr = document.cookie.match(new RegExp("(^| )"+"Nickname"+"=([^;]*)(;|$)"));//昵称未加密
		getem("inpName").value=decodeURIComponent(arr[2]);
	}
